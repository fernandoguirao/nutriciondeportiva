...
if ($cntr >= 2) {	
	for ($j=0; $j < $counters ; $j++) { 
		...
		$datass .= '{img1: "' . $aa[$j]['activitysrc'][0]  . '", img2: "' . $aa[$j]['trainersrc'][0]  . '", day: "' . $aa[$j]['day']  . '",date: "' . $aa[$j]['date']  . '",time: "' . $aa[$j]['fromto']  . '",title: "' . htmlspecialchars(preg_replace("/\r?\n/", "\\n", $ttitle), ENT_QUOTES) . '"},';
	}
...
if (count($strs) >= 2){
	foreach ($strs as $v_s) {
		...
		$data_f .= '{img: "' . $turl  . '", title: "' . htmlspecialchars(preg_replace("/\r?\n/", "\\n", $tpost->post_title), ENT_QUOTES)  . '", content: "' .  htmlspecialchars(preg_replace("/\r?\n/", "\\n", $tpost->post_content), ENT_QUOTES)  . '"},';
